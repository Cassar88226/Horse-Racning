...
dataset = pd.read_csv('feature engineering for top 3.csv', index_col=False)
print(dataset.head())
...
# Have to mention that input layer has 135 inputs. The calculation is following:

    # 2 features from race data - venue, racedistance
    # 19 horses has 7 features - horsename ,row, trainer, driver, startingodds, handicap, age
...
model_weight_name = 'model-32-64-32-8-1(all relu)-' + cur_date_time + '-' + 'epoch={epoch:02d}-val_loss={val_loss:.6f}.hdf5'
model_structure_name = 'model-32-64-32-8-1(all relu)-' + cur_date_time + '.json'
